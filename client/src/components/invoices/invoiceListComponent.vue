<template>
    <app-content-header contentHeaderTitle="Faturalar" previousTab="Faturalar" currentTab="Kayıtlar" />
    <section class="content" style="margin-left: 10px; margin-right: 10px;">
        <div class="card">
            ...
            <div class="card-body">
                <div class="form-group row">
                    <div class="col-9">
                        <button type="button" class="btn btn-dark" data-toggle="modal" data-target="#createInvoiceModal">
                            ...
                            <tr v-for="(invoice, index) in invoices" :key="invoice.id">
                                ...
                                <td>
                                    <button type="button" data-dismiss="modal" class="btn btn-outline-danger"
                                        style="margin-right: 10px;" @click="setSelectedInvoice(invoice); onDelete()">
                                    ...
                                    <button type="button" data-dismiss="modal" class="btn btn-outline-success"
                                        data-toggle="modal" data-target="#updateInvoiceModal"
        ...
        <invoiceCreateModal @newInvoiceCreated="getInvoices()"></invoiceCreateModal>
        <invoiceUpdateModal @invoiceUpdated="getInvoices()" :selectedInvoice="selectedInvoice"></invoiceUpdateModal>
...
import InvoiceUpdateModal from '@/components/invoices/invoiceUpdateModal.vue';
...
export default {
    components: {
        ...
        'invoiceUpdateModal': InvoiceUpdateModal,
        'app-content-header': AppContentHeader,
        'spinner': Spinner
    },
    data() {
        return {
            invoices: null as InvoiceListDto[] | null,
            isLoading: false,
            selectedInvoice: new InvoiceListDto()
        }
    },
    beforeMount() {
        this.getInvoices();
    ...
    methods: {
        setSelectedInvoice(invoice: InvoiceListDto) {
            this.selectedInvoice = Object.assign(new InvoiceListDto(), invoice);
            this.selectedInvoice.typeValue = this.selectedInvoice.type.value;
        },
        getInvoices() {
            this.isLoading = true;
            this.$axios.get('invoices/getall')
                .then(response => {
                    this.invoices = response.data.data as InvoiceListDto[];
                })
                .catch(error => {
        ...
        onDelete() {
            Swal.fire({
                ...
                text: `Fatura : ${this.selectedInvoice!.invoiceNumber} `,
                icon: "warning",
                    ...
                    this.$axios.post('/invoices/delete', { id: this.selectedInvoice!.id })
                        .then(() => {
                            Swal.fire({
                                ...
                                text: `Fatura başarıyla silindi. ${this.selectedInvoice!.invoiceNumber}`,
                                icon: "success"
                            });
                            this.getInvoices();
                        })
                        .catch(error => {
                            ...
                            try {
                                ...
                                errorDetail = "Fatura silinirken hata oluştu!";
                            }
                            Swal.fire({
                                title: 'Hata!',
                                text: errorDetail,
