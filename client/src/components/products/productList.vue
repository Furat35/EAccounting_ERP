<template>
    <app-content-header contentHeaderTitle="Stoklar" previousTab="Stoklar" currentTab="Kayıtlar" />
    <section class="content" style="margin-left: 10px; margin-right: 10px;">
        <div class="card">
            <div class="card-header">
                <h3 class="card-title">Stoklar Listesi</h3>
            ...
            <div class="card-body">
                <div class="form-group row">
                    <div class="col-9">
                        <button type="button" class="btn btn-dark" data-toggle="modal" data-target="#createProductModal">
                    ...
                    <table class="table table-hover">
                        ...
                        <tbody>
                            <tr v-for="(product, index) in products" :key="product.id">
                                ...
                                <td :class="product.deposit - product.withdrawal < 0 ? 'text-danger' : 'text-success'">{{ product.deposit - product.withdrawal }}</td>
                                <td>
                                    <router-link :to="`/productDetails/${product.id}`">
                                        Detay
                                    </router-link>
                                    <button type="button" data-dismiss="modal" class="btn btn-outline-danger"
                                        style="margin-right: 10px;" @click="setSelectedProduct(product); onDelete()">
        ...
        <productCreateModal @newProductCreated="getProducts()"></productCreateModal>
        <productUpdateModal @productUpdated="getProducts()" :selectedProduct="selectedProduct"></productUpdateModal>
...
export default {
    ...
    data() {
        return {
            products: null as ProductListDto[] | null,
            isLoading: false,
            selectedProduct: new ProductListDto()
        }
    },
    beforeMount() {
        this.getProducts();
    ...
    methods: {
        setSelectedProduct(product: ProductListDto) {
            this.selectedProduct = Object.assign(new ProductListDto(), product);
        },
        getProducts() {
            this.isLoading = true;
        ...
        onDelete() {
            Swal.fire({
                ...
                text: `Stok : ${this.selectedProduct!.name} `,
                icon: "warning",
                ...
                if (result.isConfirmed) {
                    this.$axios.post('/products/delete', { id: this.selectedProduct!.id })
                        .then(() => {
                            Swal.fire({
                                title: "Silme işlemi tamamlandı! ",
                                text: `Stok başarıyla silindi. ${this.selectedProduct!.name}`,
                                icon: "success"
                            });
                            this.getProducts();
                        })
                        .catch(error => {
                            ...
                            try {
                                ...
                                errorDetail = "Stok silinirken hata oluştu!";
                            }
                            Swal.fire({
                                title: 'Hata!',
                                text: errorDetail,
