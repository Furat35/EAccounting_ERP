<template>
    <app-content-header contentHeaderTitle="Cariler" previousTab="Cariler" currentTab="Kayıtlar" />
    <section class="content" style="margin-left: 10px; margin-right: 10px;">
        <div class="card">
            <div class="card-header">
                <h3 class="card-title">Cariler Listesi</h3>
                    ...
                    <table class="table table-hover">
                        ...
                        <tbody>
                            <tr v-for="(customer, index) in customers" :key="customer.id">
                                ...
                                <td>{{ customer.withdrawalAmount }}</td>
                                <!-- <td :class="customer.depositAmount - customer.withdrawalAmount < 0 ? 'text-danger' : 'text-success'">{{ customer.depositAmount - customer.withdrawalAmount }}</td> -->
                                <td>
                                    <router-link :to="`/customerDetails/${customer.id}`">
                                        Detay
                                    </router-link>
                                    <button type="button" data-dismiss="modal" class="btn btn-outline-danger"
                                        style="margin-right: 10px;" @click="setSelectedCustomer(customer); onDelete()">
        ...
        <customerCreateModal @newCustomerCreated="getCustomers()"></customerCreateModal>
        <customerUpdateModal @customerUpdated="getCustomers()" :selectedCustomer="selectedCustomer"></customerUpdateModal>
...
import CustomerUpdateModal from '@/components/customers/customerUpdateModal.vue';
...
export default {
    ...
    data() {
        return {
            customers: null as CustomerListDto[] | null,
            isLoading: false,
            selectedCustomer: new CustomerListDto()
        }
    },
    beforeMount() {
        this.getCustomers();
    ...
    methods: {
        setSelectedCustomer(customer: CustomerListDto) {
            this.selectedCustomer = Object.assign(new CustomerListDto(), customer);
        },
        getCustomers() {
            this.isLoading = true;
        ...
        onDelete() {
            Swal.fire({
                ...
                text: `Cari : ${this.selectedCustomer!.name} `,
                icon: "warning",
                ...
                if (result.isConfirmed) {
                    this.$axios.post('/customers/delete', { id: this.selectedCustomer!.id })
                        .then(() => {
                            Swal.fire({
                                title: "Silme işlemi tamamlandı! ",
                                text: `Cari başarıyla silindi. ${this.selectedCustomer!.name}`,
                                icon: "success"
                            });
                            this.getCustomers();
                        })
                        .catch(error => {
                            ...
                            try {
                                ...
                                errorDetail = "Cari silinirken hata oluştu!";
                            }
                            Swal.fire({
                                title: 'Hata!',
                                text: errorDetail,
